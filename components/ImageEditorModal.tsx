      ...
      <div className="bg-[var(--bg-secondary)] w-full max-w-4xl rounded-2xl border border-[var(--border-color)] shadow-2xl p-6 relative animate-fade-in flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-2/3 flex flex-col gap-4">
            ...
            >
                ...
                <canvas ref={bubbleCanvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />
                 {isApplyingEdit && (
             ...
             <div className="bg-[var(--bg-primary)] p-2 rounded-lg flex justify-between items-center">
                <div className="flex gap-1">
                    ...
                    <button onClick={handleMagicWandClick} className={`p-2 rounded-md ${activeTool === 'wand' ? 'bg-[var(--accent)] text-white' : 'hover:bg-[var(--bg-tertiary)]'}`} title={t('aiSelect')}><AutoSelectIcon className="w-5 h-5"/></button>
        ...
        <div className="w-full md:w-1/3 flex flex-col">
            <div className="flex justify-between items-start mb-4">
                ...
                <button onClick={onClose} className="hover:text-white"><XMarkIcon className="w-6 h-6" /></button>
            </div>
            ...
            {selectedBubble && (
                <div className="mt-4 pt-4 border-t border-[var(--border-color)] space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                        <label>{t('bubbleFill')} <input type="color" value={selectedBubble.backgroundColor} onChange={e => updateSelectedBubble({ backgroundColor: e.target.value })} className="w-full bg-transparent"/></label>
                        <label>{t('bubbleText')} <input type="color" value={selectedBubble.textColor} onChange={e => updateSelectedBubble({ textColor: e.target.value })} className="w-full bg-transparent"/></label>
                        <label>{t('bubbleBorder')} <input type="color" value={selectedBubble.borderColor} onChange={e => updateSelectedBubble({ borderColor: e.target.value })} className="w-full bg-transparent"/></label>
            ...
            <div className="flex flex-col gap-3 mt-auto pt-4">
                <div className="flex gap-3">
                    <button onClick={onClose} disabled={isApplyingEdit} className="w-full px-5 py-2.5 bg-[var(--bg-tertiary)] font-semibold rounded-full hover:bg-[var(--border-color)]">{t('cancel')}</button>
                     <button onClick={handleSaveAndClose} disabled={isApplyingEdit} className="w-full inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-green-600 font-semibold rounded-full hover:bg-green-700">
